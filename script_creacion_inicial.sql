...
AS BEGIN
	INSERT INTO SARTEN_QUE_LADRA.DetalleFactura (publicacion_codigo, detalle_concepto_id, detalle_factura_total, factura_numero)
	SELECT DISTINCT maestra.PUBLICACION_CODIGO, concepto.detalle_concepto_id, concepto.precio * concepto.cantidad, maestra.FACTURA_NUMERO
	FROM gd_esquema.Maestra maestra JOIN SARTEN_QUE_LADRA.Concepto concepto ON maestra.FACTURA_DET_PRECIO = concepto.precio 
										...
										AND maestra.FACTURA_DET_TIPO = concepto_tipo
									JOIN SARTEN_QUE_LADRA.Publicacion p ON p.publicacion_codigo = maestra.PUBLICACION_CODIGO							
...
AS BEGIN
	INSERT INTO SARTEN_QUE_LADRA.Concepto (concepto_tipo, precio, cantidad, subtotal)
	SELECT DISTINCT FACTURA_DET_TIPO, FACTURA_DET_PRECIO, FACTURA_DET_CANTIDAD, FACTURA_DET_SUBTOTAL
...
AS BEGIN
	...
	SELECT DISTINCT PAGO_MEDIO_PAGO, PAGO_TIPO_MEDIO_PAGO FROM gd_esquema.Maestra
...
AS BEGIN
	...
	SELECT DISTINCT p.id_pago, mp.id_tipo_medio_de_pago, dp.detalle_pago_id FROM gd_esquema.Maestra m 
	...
	JOIN SARTEN_QUE_LADRA.MedioPago mp ON m.PAGO_MEDIO_PAGO = mp.medio_de_pago AND
										m.PAGO_TIPO_MEDIO_PAGO = mp.tipo_medio_pago
