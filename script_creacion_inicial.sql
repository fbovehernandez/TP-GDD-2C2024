...
-- Si bien el envio tiene fecha, la venta no tiene hora determinada, pero hay que migrarlo igual
...
EXEC SARTEN_QUE_LADRA.MIGRAR_VENTA -- MIGRE ESTO 2 VECES, SI APARECEN MAS FILAS ES POR ESO
SELECT * FROM SARTEN_QUE_LADRA.Venta
...
SELECT * FROM SARTEN_QUE_LADRA.Domicilio WHERE domicilio_id	 = 28018
...
-- IMPORTANTE : ACA HAY QUE VER LO DEL VENDEDOR, YO LO MIGRE Y LE PUSE VENDEDOR ID CUANDO EN REALIDAD APUNTA A UN USUARIO, PERO EN EL DER
-- > FIGURA COMO QUE APUNTA AL VENDEDOR TAMBIEN, PERO AL PEDIR EL USUARIO, ES MEDIO CONFUSO. LA MIGRACION IGUAL NO CAMBIA MUCHO SI LO CORRIGEN
...
SELECT VENTA_FECHA FROM gd_esquema.Maestra

EXEC SARTEN_QUE_LADRA.MIGRAR_PUBLICACION

SELECT * FROM SARTEN_QUE_LADRA.Publicacion 

SELECT * FROM SARTEN_QUE_LADRA.Vendedor v where v.vendedor_id = 32

SELECT DISTINCT PUBLICACION_CODIGO, PUBLICACION_STOCK, PUBLICACION_PRECIO, PRODUCTO_CODIGO, PRODUCTO_PRECIO, VENDEDOR_RAZON_SOCIAL, VENDEDOR_MAIL,
VENDEDOR_CUIT, ALMACEN_CODIGO	from gd_esquema.Maestra
WHERE PUBLICACION_CODIGO IS NOT NULL AND PRODUCTO_CODIGO IS NOT NULL

SELECT * FROM SARTEN_QUE_LADRA.Producto WHERE producto_codigo = 'Codigo:1131231312' -- 1009
SELECT * FROM SARTEN_QUE_LADRA.Usuario WHERE usuario_nombre = 'Razon Social Nº: 24123132136513510' -- 33743
	AND usuario_fecha_creacion = '2004-05-11'
...
where VENTA_CODIGO IS NOT NULL PUBLICACION_CODIGO IS NOT NULL
...
SELECT DISTINCT * FROM SARTEN_QUE_LADRA.Concepto c
ORDER BY c.precio 

SELECT DISTINCT FACTURA_DET_CANTIDAD, FACTURA_DET_PRECIO, FACTURA_DET_SUBTOTAL, FACTURA_DET_TIPO FROM gd_esquema.Maestra
WHERE FACTURA_DET_TIPO IS NOT NULL
ORDER BY FACTURA_DET_PRECIO
